...
from tests.test_v1469_paid_execution_claim_repository import (
    ARM_KEY, ENVIRONMENT, LEASE_ID, SYMBOL,
...
def test_019_rebuild_preserves_rows_and_enables_submission_lifecycle(tmp_path: Path):
    async def scenario():
        ...
        terminal_seed = await _transition_claim_to_submitted(
            repo, terminal_seed, submitting_at_ms=1125,
            submitted_at_ms=1150, key="migration-terminal",
        )
        terminal = (await repo.terminalize_claim(claim_id=terminal_seed.claim_id,
            expected_generation=terminal_seed.generation,
            terminal_at_ms=1200, terminal_reason="CLOSED",
        ...
        assert (await upgraded.fetchone("SELECT COUNT(*) AS n FROM v1469_paid_execution_claim_events"))["n"] == 9
