...
async def test_entry_fill_syncs_maker_take_profit_orders():
    ...
    assert len(client.reduce_only_limit_orders) == 3
    assert all(order["side"] == "SELL" for order in client.reduce_only_limit_orders)
    assert {order["clientOrderId"] for order in client.reduce_only_limit_orders} == {"cry3mn_test_tp1", "cry3mn_test_tp2", "cry3mn_test_tp3"}
...
async def test_run_running_stop_loss_closes_with_market_and_cancels_tp_orders():
    ...
    # TP limit orders cancelled via cancel_order (tp1 + tp2 + tp3)
