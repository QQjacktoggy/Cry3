...
START_DATE="${START_DATE:-2025-11-14}"
END_DATE="${END_DATE:-2026-05-13}"
LABEL="${LABEL:-router_allocator_v13_trend350_ai_risk_auto_target10_cap100}"
WINDOW_DAYS="${WINDOW_DAYS:-30}"
STEP_DAYS="${STEP_DAYS:-15}"
...
echo "[CANDIDATE] python=${PYTHON_BIN}"
echo "[CANDIDATE] dates=${START_DATE}..${END_DATE}"
...
"${PYTHON_BIN}" scripts/segmented_backtest.py \
  ...
  --step-days "${STEP_DAYS}" \
  --output-dir "testnet/results/segmented_backtests/${LABEL}_rolling_w${WINDOW_DAYS}_s${STEP_DAYS}" \
  -- \
  "${COMMON_ARGS[@]}"
